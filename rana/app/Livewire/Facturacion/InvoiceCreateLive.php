...
class InvoiceCreateLive extends Component
{
    ...
    public function emitFactura()
    {
        ...
        if ($this->docAdjunto) {
            ...
            switch ($firstChar) {
                case 'F':
                    $this->docAdjunto_type = 'Factura';
                    break;
                case 'B':
                    $this->docAdjunto_type = 'Boleta';
                    break;
                case 'T':
                    $this->docAdjunto_type = 'Guía de Remisión';
                    break;
                case 'V':
                    $this->docAdjunto_type = 'Guía de Transportista';
                    break;
                default:
                    $this->docAdjunto_type = 'Otro documento';
                    break;
            }
        }
        ...
        if ($this->total >= 400 && $this->tipoOperacion == '1001') {
            $invoiceData['codBienDetraccion'] = $this->tipoDetraccion;
            $invoiceData['codMedioPago'] = '001';
            $invoiceData['ctaBanco'] = $company->ctaBanco;
            $invoiceData['setPercent'] = 4;
            $invoiceData['setMount'] = $this->total * 0.04;
            $legends[] = [
                'code' => '2006',
        ...
        $invoiceData['legends'] = json_encode($legends);
        ...
        try {
            $factura = $this->invoiceService->createInvoice($invoiceData, $detailsData);
            ...
            $this->error('Error al emitir factura: ' . $e->getMessage());
        }
    }
