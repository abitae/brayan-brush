<div class="p-4 w-full max-w-7xl mx-auto space-y-4">
    <div class="bg-white rounded-lg shadow-lg border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-4">
            ...
            <flux:button href="{{ route('facturacion.note.create') }}" variant="primary" icon="plus">
                Nuevo
            </flux:button>
        </div>
            ...
            <form wire:submit.prevent class="w-full">
                <div class="flex flex-col md:flex-row md:items-end md:space-x-4 gap-4 flex-wrap">
                    <div class="flex-1 min-w-[200px]">
                        <flux:input type="search" label="Buscar nota" wire:model.live="search"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($notes as $note)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 text-sm">{{ $note->id }}</td>
                            <td class="px-4 py-3 text-sm">
                                <div class="font-semibold text-purple-600">
                                    {{ $note->serie }}-{{ $note->correlativo }}</div>
                                <div class="text-xs text-gray-500">{{ $note->created_at->format('d-m-Y H:i') }}</div>
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <div class="text-xs text-gray-500">{{ $note->client->code }}</div>
                                <div class="font-medium">{{ $note->client->name }}</div>
                            </td>
                            <td class="px-4 py-3 text-sm font-semibold">S/ {{ number_format($note->mtoImpVenta, 2) }}</td>
                            <td class="px-4 py-3 text-sm">
                                <div class="flex gap-1">
                                    <flux:button wire:click="showPdf({{ $note->id }}, 'a4')"
                                        size="xs" variant="primary" icon="document-text" />
                                    <flux:button wire:click="showPdf({{ $note->id }}, '80mm')"
                                        size="xs" variant="primary" icon="printer" />
                                </div>
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <div class="flex gap-1">
                                    @if($note->xml_path && $note->xml_hash)
                                        <flux:button wire:click="xmlDownload({{ $note->id }})" size="xs" variant="primary" icon="arrow-down-tray" />
                                    @else
                                        <flux:button wire:click="xmlGenerate({{ $note->id }})" size="xs" variant="primary" icon="arrow-path" />
                                    @endif
                                    @if($note->cdr_path)
                                        <flux:button wire:click="downloadCdrFile({{ $note->id }})" size="xs" variant="primary" icon="arrow-down-tray" />
                                    @else
                                        <flux:button wire:click="sendXmlFile({{ $note->id }})" size="xs" variant="primary" icon="paper-airplane" />
                                    @endif
                                </div>
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        <flux:menu.item wire:click="showInfo({{ $note->id }})"
    ...
    <flux:modal wire:model="pdfModal" name="pdfModal" class="w-full max-w-2xl mx-auto">
        <div class="p-4 space-y-3">
            <div class="flex items-center justify-between pb-2 border-b border-gray-100 dark:border-zinc-700">
                ...
                @if ($pdfUrl)
                    <a href="{{ $pdfUrl }}" target="_blank" class="flex items-center gap-1 px-2 py-1 text-xs text-blue-600 hover:underline" title="Abrir PDF en nueva pestaña">
            ...
            <div>
                <div id="loadingStateNotePdf" class="flex items-center justify-center py-8">
                    <div>
                ...
                <div id="pdfContainerNotePdf" class="hidden">
                    <div class="rounded border border-gray-100 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-2">
                        <div class="flex justify-end mb-1">
                            <button type="button" onclick="toggleFullscreen_pdfFrameNotePdf()" class="text-gray-400 dark:text-zinc-500 hover:text-gray-600 text-xs flex items-center gap-1" title="Pantalla completa">
                                <flux:icon name="arrows-pointing-out" class="w-4 h-4" />
                            </button>
                        </div>
                        @if ($pdfUrl)
                            <iframe id="pdfFrameNotePdf" src="{{ $pdfUrl }}" class="w-full border-0" style="min-height: 400px; max-height: 60vh;" onload="hideLoading_loadingStateNotePdf()" onerror="showError_errorStateNotePdf()"></iframe>
                        @endif
                    </div>
                </div>
                <div id="errorStateNotePdf" class="hidden">
                    <div class="text-center py-8">
                        ...
                        <div class="flex justify-center gap-2">
                            <flux:button wire:click="refreshPdf" icon="arrow-path" size="xs" class="bg-blue-500 text-white">Reintentar</flux:button>
                            @if ($pdfUrl)
                                <a href="{{ $pdfUrl }}" target="_blank" class="flex items-center gap-1 px-2 py-1 text-xs text-blue-600 hover:underline">
                                    <flux:icon name="arrow-top-right-on-square" class="w-4 h-4" />
                                    PDF
                                </a>
                            @endif
            ...
            <div class="flex justify-end pt-2 border-t border-gray-100 dark:border-zinc-700">
                <flux:button wire:click="closePdf" size="xs" class="text-gray-500 dark:text-zinc-400 hover:text-gray-700">Cerrar</flux:button>
            </div>
        </div>
    </flux:modal>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            function initPdfModalNote(config) {
                var loadingId = config.loadingId, containerId = config.containerId, errorId = config.errorId, frameId = config.frameId, refreshEvent = config.refreshEvent;
                window['hideLoading_' + loadingId] = function() {
                    var loadingEl = document.getElementById(loadingId), containerEl = document.getElementById(containerId);
                    if (loadingEl) loadingEl.classList.add('hidden');
                    if (containerEl) containerEl.classList.remove('hidden');
                };
                window['showError_' + errorId] = function() {
                    var loadingEl = document.getElementById(loadingId), errorEl = document.getElementById(errorId);
                    if (loadingEl) loadingEl.classList.add('hidden');
                ...
                document.addEventListener('livewire:init', function() {
                    Livewire.on(refreshEvent, function() {
                        var errorEl = document.getElementById(errorId), loadingEl = document.getElementById(loadingId), containerEl = document.getElementById(containerId), iframe = document.getElementById(frameId);
                        if (errorEl) errorEl.classList.add('hidden');
                        if (loadingEl) loadingEl.classList.remove('hidden');
                        if (containerEl) containerEl.classList.add('hidden');
                        if (iframe) iframe.src = iframe.src;
                    });
                    Livewire.on('note-pdf-opening', function() {
                        var loadingEl = document.getElementById(loadingId), containerEl = document.getElementById(containerId), errorEl = document.getElementById(errorId);
                        if (loadingEl) loadingEl.classList.remove('hidden');
                        if (containerEl) containerEl.classList.add('hidden');
                        if (errorEl) errorEl.classList.add('hidden');
                    });
            ...
            initPdfModalNote({ loadingId: 'loadingStateNotePdf', containerId: 'pdfContainerNotePdf', errorId: 'errorStateNotePdf', frameId: 'pdfFrameNotePdf', refreshEvent: 'note-pdf-refreshed' });
        });
    </script>
</div>
