...
<body>
    <table class="table header">
        <tr>
            <td style="width: 40%;">
                @php
                    $logoPath = $company->logo_path ?? 'img/logo_completo.png';
                    $logoFullPath = public_path($logoPath);
                    if (!is_file($logoFullPath)) {
                        $logoFullPath = public_path('img/logo_completo.png');
                    }
                    $logoData = '';
                    if (is_file($logoFullPath)) {
                        $ext = strtolower(pathinfo($logoFullPath, PATHINFO_EXTENSION));
                        $ext = $ext === 'jpg' ? 'jpeg' : $ext;
                        $logoData = 'data:image/' . $ext . ';base64,' . base64_encode(file_get_contents($logoFullPath));
                    }
                @endphp
                @if ($logoData)
                    <img class="header__logo" src="{{ $logoData }}" alt="Logo">
                @endif
            </td>
            <td>
                <div class="header__company">
                    <div class="header__title">
                        {{ $company->razonSocial ?? 'Empresa no registrada' }}
                    </div>
                    <div class="header__title">
                        {{ $company->ruc ?? 'RUC no registrado' }}
                    </div>
                    <div class="header__info">
                        {{ $company->address ?? ($encomienda->sucursal_remitente->address ?? '-') }}<br>
                        {{ $company->phone ?? '-' }}<br>
                        {{ $company->email ?? '-' }}
                    </div>
                </div>
    ...
    <div class="document">
        ...
        <div class="document__section">
            ...
            de la empresa <span class="text-bold">{{ $company->razonSocial ?? 'Empresa no registrada' }}</span>, corresponde a:
        </div>
        ...
        <div class="document__section mt-2">
            ...
            <div><span class="text-bold">FECHA Y HORA:</span> {{ $encomienda->fecha_creacion?->format('d/m/Y H:i') ?? '-' }}</div>
        </div>
