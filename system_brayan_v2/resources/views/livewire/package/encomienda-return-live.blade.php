<div class="p-4 w-full max-w-7xl mx-auto space-y-4">
    <x-package.encomienda-page-header
        title="Retorno de Encomiendas"
        description="Procesa el retorno de encomiendas recibidas"
        icon="arrow-uturn-left"
        iconColor="red"
        actionLabel="Retornar"
        actionMethod="openReturnModal()"
        actionIcon="arrow-uturn-left"
        :actionDisabled="$this->selectedEncomiendasCount === 0"
        :actionCount="$this->selectedEncomiendasCount" />

    <x-package.encomienda-table-wrapper borderColor="red">
        <x-slot name="filters">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4 border-b border-gray-100 dark:border-zinc-700 bg-red-50/30 dark:bg-red-900/20">
                <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        <flux:input 
                            ...
                            size="xs"
                            class="w-full sm:w-64 max-w-full focus:border-red-400 dark:focus:border-red-500 transition" />
                    </div>
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        <label class="text-xs text-gray-600 dark:text-zinc-400 font-medium hidden sm:block">
                        ...
                        <flux:input 
                            ...
                            size="xs"
                            class="w-full sm:w-36 max-w-full focus:border-red-400 dark:focus:border-red-500 transition" />
                    </div>
                ...
                <tbody>
                    @forelse($encomiendas as $i => $encomienda)
                        <tr wire:key="encomienda-{{ $encomienda->id }}" class="hover:bg-gray-50 dark:hover:bg-zinc-800 transition">
                            ...
                            <x-package.encomienda-code-cell :encomienda="$encomienda" color="red" :showPago="true" :showComprobante="true" />
                            <x-package.encomienda-people-cell :encomienda="$encomienda" />
                            <x-package.encomienda-destination-cell 
                                :encomienda="$encomienda" 
                                :showFechaCreacion="true" />
                            <x-package.encomienda-amount-cell :amount="$encomienda->monto ?? 0" color="red" />
                            <td class="px-2 py-1 text-center">
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        ...
                                        @elseif($encomienda->estado_pago === 'CONTRA ENTREGA')
                                            <flux:menu.item icon="currency-dollar"
                                                wire:click="openCobroModal({{ $encomienda->id }})">
                                                Cobrar Encomienda
                                            </flux:menu.item>
                                        @endif
    ...
    @if($encomiendaSeleccionada)
        <flux:modal name="return" wire:model="showReturnModal">
            <form wire:submit="returnEncomienda">
                <div class="space-y-4">
                    <div>
                        <h2 class="text-base font-semibold text-zinc-900 dark:text-white">Retornar Encomienda</h2>
                        <p class="text-xs text-zinc-500 dark:text-zinc-400">Código: {{ $encomiendaSeleccionada->code }}</p>
                    </div>

                    <x-package.encomienda-info-card :encomienda="$encomiendaSeleccionada" />

                    <flux:textarea wire:model="motivo_retorno" label="Motivo del Retorno" 
                        required rows="4" placeholder="Describa el motivo del retorno (mínimo 10 caracteres)" />

                    <flux:input type="datetime-local" wire:model="fecha_retorno" label="Fecha de Retorno" required />

                    <div class="flex justify-end gap-2">
                        <flux:button type="button" wire:click="closeReturnModal()" variant="ghost" size="xs">Cancelar</flux:button>
                        <flux:button type="submit" variant="danger" size="xs">Confirmar Retorno</flux:button>
                    </div>
                </div>
            </form>
        </flux:modal>
    ...
    <flux:modal wire:model="modalImprimirTicket" :dismissible="true"
        ...
        <div class="p-4 space-y-3">
            ...
            <div>
                <!-- Estado de carga -->
                <div id="loadingStateTicketReturn" class="flex items-center justify-center py-8">
                    <div>
                ...
                <!-- PDF -->
                <div id="pdfContainerTicketReturn" class="hidden">
                    <div class="rounded border border-gray-100 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-2">
                        <div class="flex justify-end mb-1">
                            <button type="button" onclick="toggleFullscreenTicketReturn()"
                        ...
                        <iframe id="pdfFrameTicketReturn"
                            src="{{ route('encomienda.ticket.pdf', ['id' => $encomienda_id ?? 0]) }}"
                            class="w-full border-0" style="min-height: 400px; max-height: 60vh;"
                            onload="hideLoadingTicketReturn()" onerror="showErrorTicketReturn()">
                        </iframe>
                    </div>
                </div>

                <!-- Error -->
                <div id="errorStateTicketReturn" class="hidden">
        ...
        <script>
            function hideLoadingTicketReturn() {
                document.getElementById('loadingStateTicketReturn').classList.add('hidden');
                document.getElementById('pdfContainerTicketReturn').classList.remove('hidden');
            }

            function showErrorTicketReturn() {
                document.getElementById('loadingStateTicketReturn').classList.add('hidden');
                document.getElementById('errorStateTicketReturn').classList.remove('hidden');
            }

            function toggleFullscreenTicketReturn() {
                const iframe = document.getElementById('pdfFrameTicketReturn');
            ...
            setTimeout(function() {
                if (!document.getElementById('pdfContainerTicketReturn').classList.contains('hidden')) {
                    hideLoadingTicketReturn();
                }
            }, 10000);
            // Livewire: refrescar ticket
            document.addEventListener('livewire:init', () => {
                Livewire.on('ticket-refreshed', () => {
                    document.getElementById('errorStateTicketReturn').classList.add('hidden');
                    document.getElementById('loadingStateTicketReturn').classList.remove('hidden');
                    document.getElementById('pdfContainerTicketReturn').classList.add('hidden');
                    const iframe = document.getElementById('pdfFrameTicketReturn');
                    if (iframe) {
    ...
    <flux:modal wire:model="modalVerInvoice" :dismissible="true"
        ...
        <div class="p-4 space-y-3">
            ...
            <div>
                <!-- Estado de carga -->
                <div id="loadingStateInvoiceReturn" class="flex items-center justify-center py-8">
                    <div>
                ...
                <!-- PDF -->
                <div id="pdfContainerInvoiceReturn" class="hidden">
                    <div class="rounded border border-gray-100 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-2">
                        <div class="flex justify-end mb-1">
                            <button type="button" onclick="toggleFullscreenInvoiceReturn()"
                                class="text-gray-400 dark:text-zinc-500 hover:text-gray-600 text-xs flex items-center gap-1"
                                title="Pantalla completa">
                                <flux:icon name="arrows-pointing-out" class="w-4 h-4" />
                            </button>
                        </div>
                        <iframe id="pdfFrameInvoiceReturn"
                            src="{{ route('pdf.invoice.80mm', ['invoice' => $invoice_id ?? 0]) }}"
                            class="w-full border-0" style="min-height: 400px; max-height: 60vh;"
                            onload="hideLoadingInvoiceReturn()" onerror="showErrorInvoiceReturn()">
                        </iframe>
                    </div>
                </div>

                <!-- Error -->
                <div id="errorStateInvoiceReturn" class="hidden">
                    <div class="text-center py-8">
                        <flux:icon name="exclamation-triangle" class="w-10 h-10 text-red-400 mx-auto mb-2" />
        ...
        <!-- Scripts minimalistas -->
        <script>
            function hideLoadingInvoiceReturn() {
                document.getElementById('loadingStateInvoiceReturn').classList.add('hidden');
                document.getElementById('pdfContainerInvoiceReturn').classList.remove('hidden');
            }

            function showErrorInvoiceReturn() {
                document.getElementById('loadingStateInvoiceReturn').classList.add('hidden');
                document.getElementById('errorStateInvoiceReturn').classList.remove('hidden');
            }

            function toggleFullscreenInvoiceReturn() {
                const iframe = document.getElementById('pdfFrameInvoiceReturn');
                if (iframe.requestFullscreen) {
                    iframe.requestFullscreen();
                } else if (iframe.webkitRequestFullscreen) {
                    iframe.webkitRequestFullscreen();
            ...
            // Fallback para mostrar PDF si tarda mucho
            setTimeout(function() {
                if (!document.getElementById('pdfContainerInvoiceReturn').classList.contains('hidden')) {
                    hideLoadingInvoiceReturn();
                }
            }, 10000);
            // Livewire: refrescar invoice
            document.addEventListener('livewire:init', () => {
                Livewire.on('invoice-refreshed', () => {
                    document.getElementById('errorStateInvoiceReturn').classList.add('hidden');
                    document.getElementById('loadingStateInvoiceReturn').classList.remove('hidden');
                    document.getElementById('pdfContainerInvoiceReturn').classList.add('hidden');
                    const iframe = document.getElementById('pdfFrameInvoiceReturn');
                    if (iframe) {
                        iframe.src = iframe.src;
                    }
    ...
    <flux:modal wire:model="modalVerGuia" :dismissible="true"
        ...
        <div class="p-4 space-y-3">
            ...
            <div>
                <!-- Estado de carga -->
                <div id="loadingStateGuiaReturn" class="flex items-center justify-center py-8">
                    <div>
                ...
                <!-- PDF -->
                <div id="pdfContainerGuiaReturn" class="hidden">
                    <div class="rounded border border-gray-100 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-2">
                        <div class="flex justify-end mb-1">
                            <button type="button" onclick="toggleFullscreenGuiaReturn()"
                                class="text-gray-400 dark:text-zinc-500 hover:text-gray-600 text-xs flex items-center gap-1"
                                title="Pantalla completa">
                                <flux:icon name="arrows-pointing-out" class="w-4 h-4" />
                            </button>
                        </div>
                        <iframe id="pdfFrameGuiaReturn"
                            src="{{ route('encomienda.guia.pdf', ['id' => $guia_id ?? 0]) }}"
                            class="w-full border-0" style="min-height: 400px; max-height: 60vh;"
                            onload="hideLoadingGuiaReturn()" onerror="showErrorGuiaReturn()">
                        </iframe>
                    </div>
                </div>

                <!-- Error -->
                <div id="errorStateGuiaReturn" class="hidden">
                    <div class="text-center py-8">
                        <flux:icon name="exclamation-triangle" class="w-10 h-10 text-red-400 mx-auto mb-2" />
        ...
        <!-- Scripts minimalistas -->
        <script>
            function hideLoadingGuiaReturn() {
                document.getElementById('loadingStateGuiaReturn').classList.add('hidden');
                document.getElementById('pdfContainerGuiaReturn').classList.remove('hidden');
            }

            function showErrorGuiaReturn() {
                document.getElementById('loadingStateGuiaReturn').classList.add('hidden');
                document.getElementById('errorStateGuiaReturn').classList.remove('hidden');
            }

            function toggleFullscreenGuiaReturn() {
                const iframe = document.getElementById('pdfFrameGuiaReturn');
                if (iframe.requestFullscreen) {
                    iframe.requestFullscreen();
                } else if (iframe.webkitRequestFullscreen) {
                    iframe.webkitRequestFullscreen();
            ...
            // Fallback para mostrar PDF si tarda mucho
            setTimeout(function() {
                if (!document.getElementById('pdfContainerGuiaReturn').classList.contains('hidden')) {
                    hideLoadingGuiaReturn();
                }
            }, 10000);
            // Livewire: refrescar guía
            document.addEventListener('livewire:init', () => {
                Livewire.on('guia-refreshed', () => {
                    document.getElementById('errorStateGuiaReturn').classList.add('hidden');
                    document.getElementById('loadingStateGuiaReturn').classList.remove('hidden');
                    document.getElementById('pdfContainerGuiaReturn').classList.add('hidden');
                    const iframe = document.getElementById('pdfFrameGuiaReturn');
                    if (iframe) {
                        iframe.src = iframe.src;
                    }
    ...
    @if($encomiendaDetalle)
        <flux:modal wire:model="showDetailsModal" :dismissible="true" max-width="4xl">
            <div class="px-4 py-3 border-b border-zinc-100 dark:border-zinc-700">
                <div class="flex items-center gap-2">
                    <div class="w-6 h-6 bg-red-50 dark:bg-red-900/20 rounded flex items-center justify-center">
                        <flux:icon name="information-circle" class="w-4 h-4 text-red-600 dark:text-red-400" />
                    </div>
            ...
            <div class="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-white dark:bg-zinc-900 rounded-lg shadow border p-3 text-xs space-y-2">
                        <div class="font-semibold text-red-700 dark:text-red-400 mb-2">Información General</div>
                    ...
                    <div class="bg-white dark:bg-zinc-900 rounded-lg shadow border p-3 text-xs space-y-2">
                        <div class="font-semibold text-red-700 dark:text-red-400 mb-2">Remitente</div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-white dark:bg-zinc-900 rounded-lg shadow border p-3 text-xs space-y-2">
                        <div class="font-semibold text-red-700 dark:text-red-400 mb-2">Destinatario</div>
                        ...
                        <div><span class="font-semibold">Sucursal:</span> {{ $encomiendaDetalle->sucursal_destinatario->name ?? 'N/A' }}</div>
                    </div>

                    @if($encomiendaDetalle->ruta)
                        <div class="bg-white dark:bg-zinc-900 rounded-lg shadow border p-3 text-xs space-y-2">
                            <div class="font-semibold text-red-700 dark:text-red-400 mb-2">Ruta</div>
                            ...
                            <div><span class="font-semibold">Vehículo:</span> {{ $encomiendaDetalle->ruta->vehiculo->placa ?? 'N/A' }}</div>
                ...
                @if($encomiendaDetalle->paquetes && $encomiendaDetalle->paquetes->count() > 0)
                    <div class="bg-white dark:bg-zinc-900 rounded-lg shadow border p-3 text-xs">
                        <div class="font-semibold text-red-700 dark:text-red-400 mb-2">Paquetes ({{ $encomiendaDetalle->paquetes->count() }})</div>
                        <div class="overflow-x-auto">
                                ...
                                <tbody>
                                    @foreach($encomiendaDetalle->paquetes as $paquete)
                                        <tr class="border-b border-gray-100 dark:border-zinc-800">
                                            ...
                                            <td class="text-right py-1">{{ number_format($paquete->peso ?? 0, 2) }} kg</td>
                                            <td class="text-right py-1">S/ {{ number_format($paquete->amount ?? 0, 2) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-white dark:bg-zinc-900 rounded-lg shadow border p-3 text-xs space-y-2">
                        <div class="font-semibold text-red-700 dark:text-red-400 mb-2">Información Adicional</div>
                        ...
                        <div><span class="font-semibold">Comprobante:</span> {{ $encomiendaDetalle->tipo_comprobante ?? 'N/A' }}</div>
                        <div><span class="font-semibold">Entrega a Domicilio:</span> {{ $encomiendaDetalle->isHome ? 'Sí' : 'No' }}</div>
                        <div><span class="font-semibold">Retorno:</span> {{ $encomiendaDetalle->isReturn ? 'Sí' : 'No' }}</div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-emerald-50 dark:bg-emerald-900/20 p-3 rounded-lg">
                        <div class="text-xs space-y-2">
                            <div><span class="font-semibold">Monto a cobrar:</span> S/ {{ number_format($encomiendaCobro->monto ?? 0, 2) }}</div>
                            <div><span class="font-semibold">Cliente:</span> {{ $encomiendaCobro->destinatario->name ?? 'N/A' }}</div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-2">
                        <flux:button type="button" wire:click="closeCobroModal()" variant="ghost" size="xs">Cancelar</flux:button>
                        <flux:button type="submit" variant="primary" size="xs" color="emerald">Procesar Cobro</flux:button>
                    </div>
                </div>
            </form>
        </flux:modal>
