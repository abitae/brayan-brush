...
export const DEFAULT_PAGE_CONTENT: PageContent = {
  hero: {
    ...
    image_caption_text:
      'Flota moderna y segura para mover tu carga con puntualidad en todo el territorio peruano.',
  ...
  portfolio: {
    ...
    subtitle:
      'Soluciones logísticas para paquetería, mudanzas y carga pesada. Elige el servicio que se adapta a tu operación y cotiza en minutos.',
    ...
    cta_text:
      'Consulta todos nuestros servicios, compara opciones y solicita una cotización personalizada.',
  },
  about: {
    title: 'Liderazgo en la ruta nacional peruana.',
    text_1:
      'es más que una empresa de transporte; somos su socio estratégico. Gracias a nuestro panel de control centralizado, gestionamos cada unidad con precisión quirúrgica.',
    text_2:
  ...
  contact: {
    ...
    subtitle:
      'Completa el formulario y uno de nuestros asesores expertos te enviará una propuesta personalizada de inmediato.',
  },
  tracking: {
    ...
    empty_text:
      'Estados disponibles: Registrado, Enviado, Recibido, Entregado o Retornado. Completa el formulario para ver el detalle y el historial del envío.',
  },
  prohibited: {
    ...
    warning_text_1:
      'El envío de artículos prohibidos está sujeto a sanciones legales y puede resultar en la cancelación del servicio sin derecho a reembolso.',
    warning_text_2:
      'Si tiene dudas sobre algún artículo específico, por favor contáctenos antes de realizar el envío para recibir asesoría personalizada.',
...
export function usePageContent(): PageContent {
  const siteConfig = usePage().props.siteConfig as { page_content?: PageContent } | null;
  return deepMerge(DEFAULT_PAGE_CONTENT, siteConfig?.page_content);
}

export function useAgencies() {
  const agencies = usePage().props.agencies as
    | { id: string; name: string; address: string; city: string; phone: string; lat: number; lng: number }[]
    | undefined;
  return agencies ?? [];
}
