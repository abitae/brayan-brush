...
import { AGENCIES, ICONS } from '../constants';
    ...
    <section className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {AGENCIES.map((agency) => (
            <div key={agency.id} className="bg-slate-50 border border-slate-100 p-8 rounded-3xl hover:bg-emerald-50 hover:border-emerald-200 transition-all group">
              ...
              <p className="text-slate-500 text-sm mb-4 min-h-[40px]">{agency.address}, {agency.city}</p>
              <div className="flex items-center gap-2 text-emerald-700 font-bold text-sm mb-6">
                <span>📞</span> {agency.phone}
              </div>
              <button 
                onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${agency.lat},${agency.lng}`)}
        ...
        <div className="mt-20 bg-emerald-900 rounded-[40px] p-12 text-white relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="max-w-xl">
              ...
              <p className="text-emerald-100 opacity-80">Estamos expandiéndonos en todas las provincias del Perú. Si tienes una empresa de transporte local, únete como agencia aliada.</p>
            </div>
            <button className="bg-emerald-500 hover:bg-emerald-400 text-white px-10 py-5 rounded-2xl font-black transition-all shadow-xl whitespace-nowrap">
